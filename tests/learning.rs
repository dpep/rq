...
fn scratch_dir() -> PathBuf {
    let dir = std::env::temp_dir().join(format!("rq-learn-{}", std::process::id()));
...
fn a_selection_changes_future_ranking() {
    ...
    let before = search::search(&store, "handler", None, 10).unwrap();
    ...
    let after = search::search(&store, "handler", None, 10).unwrap();
